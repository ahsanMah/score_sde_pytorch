...
def get_sampling_fn(config, sde, shape, inverse_scaler, eps):
    ...
    # Predictor-Corrector sampling. Predictor-only and Corrector-only samplers are special cases.
        ...
        sampling_fn = get_pc_sampler(
            ...
            denoise=config.sampling.noise_removal,
            eps=eps,
            masked_marginals=config.data.mask_marginals,
            device=config.device,
        )
    else:
...
def get_pc_sampler(
    ...
    def pc_sampler(model):
        ...
        with torch.no_grad():
            ...
            if masked_marginals:
                # Single channel mask for fully observed marginals
                mask = torch.ones((shape[0], 1, *shape[2:])).to(device)
                x[:, -1:, ...] = mask
            else:
                mask = None
            ...
            for i in range(sde.N):
                ...
                # if masked_marginals and not isinstance(corrector, NoneCorrector):
